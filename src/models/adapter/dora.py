...
class DoraInjectedLinear(nn.Module):
    def __init__(
        ...
        self.lora_down = nn.Parameter(torch.randn(r, in_features) * std_dev)
        self.lora_up = nn.Parameter(torch.zeros(out_features, r))
    ...
    def set_dora(self):
        self.weight = nn.Parameter(self.linear.weight.clone(),requires_grad=False)
        
        if self.linear.bias is not None:
            self.bias = nn.Parameter(self.linear.bias.clone(), requires_grad=False)
        else:
            self.bias = None
