...
def extract_lora_ups_down(model, target_replace_module=DEFAULT_TARGET_REPLACE):
    ...
    for _m, _n, _child_module in _find_modules(
        ...
        search_class=[DoraInjectedLinear, LoraInjectedConv2d],
    ):
...
def inspect_lora(model):
    ...
    for name, _module in model.named_modules():
        if _module.__class__.__name__ in ["DoraInjectedLinear", "LoraInjectedConv2d"]:
...
def save_all(
    ...
    if not safe_form:
        ...
        if save_lora:
            ...
            save_lora_weight(
                unet, save_path+'unet_dora.pt', target_replace_module=target_replace_module_unet
            )
            print("Unet saved to ", save_path)

            save_lora_weight(
                text_encoder,
                save_path+'encoder1_dora.pt',
                target_replace_module=target_replace_module_text,
            )
            print("Text Encoder1 saved to ", save_path+'encoder1_dora.pt')

            save_lora_weight(
                text_encoder2,
                save_path+'encoder2_dora.pt',
                target_replace_module=target_replace_module_text,
            )
            print("Text Encoder2 saved to ", save_path+'encoder2_dora.pt')
...
def monkeypatch_or_replace_lora(
    ...
    for _module, name, _child_module in _find_modules(
        model, target_replace_module, search_class=[nn.Linear, DoraInjectedLinear]
    ):
        _source = (
            _child_module.linear
            if isinstance(_child_module, DoraInjectedLinear)
            else _child_module
        )
